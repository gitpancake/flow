...
const check = function () {
  if (typeof fxr !== "function")
    if (typeof fxrand !== "function") throw new Error("fxhash has not been defined, did you call FXInit? Is the fx hash snippet included?");
    else FXInit(fxrand);
};
...
export const getWeightedOption = function (options) {
  check();
  let choices = [];
  for (let i in options) choices = choices.concat(new Array(options[i][1]).fill(options[i][0]));
  return pick(choices);
};
...
export const FXRandomReset = (newhash) => {
  ...
  fxhash = newhash
    ...
    : "oo" +
      Array(49)
        .fill(0)
        .map((_) => alphabet[(fxr() * alphabet.length) | 0])
        .join("");
